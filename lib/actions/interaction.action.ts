...
export async function createInteraction(
  ...
  try {
    ...
    // Update reputation for both the user(performer) and the target (authorId)
    await updateRepution({
      interaction,
      session,
      performerId: userId!,
      authorId,
...
async function updateRepution(params: UpdateReputationParams) {
  const { interaction, session, performerId, authorId } = params;
  const { action, actionType } = interaction;

  // Update the reputation of the performer
